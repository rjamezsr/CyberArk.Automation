...
# If not verifying SSL, this script should be used for testing environment only and assumes that SSL verification can be bypassed. Please verify SSL certificate before using in production 
# SUPPORTED VERSIONS:
...
(
	...
	[Alias("url")]
	[String]$PVWAURL="https://cxcorp-pvwa04.cxcorp.local/passwordvault",
	...
	[Parameter(Mandatory=$false,HelpMessage="Please enter a filter statement")]
	...
	# Use this switch to disable warning about duplicate accounts
	[Parameter(ParameterSetName='Create',Mandatory=$false)]
	[Parameter(ParameterSetName='Update')]
	[Switch]$Force
...
# Set Log file path
$LOG_FILE_PATH = "$ScriptLocation\logs\Account_Onboarding_Utility.log"
$Log = $LOG_FILE_PATH
	...
	try{
		...
		switch ($type)
		{
			...
			"Warning" {
				...
				$msgToWrite += "[WARNING]" + $Msg
			}
			"Error" {
				Write-Verbose $MSG.ToString() 
				#$msgToWrite += "[ERROR]`t$Msg"
				$msgToWrite += "[ERROR]" + $Msg
			}
			"Debug" { 
				if($InDebug)
				{
...
Function Get-Safe
{
	...
	try{
		...
		#$_safe = $(Invoke-RestMethod -Uri $accSafeURL -Header $g_LogonHeader -Method "Get" -ErrAction $ErrAction)
		$_safe = $(Invoke-RestMethod -Uri $accSafeURL -Header $g_LogonHeader -Method "Get")
	}
	catch
	{
		#Log-Msg -Type Error -MSG $_.Exception.Response.StatusDescription
		Log-Msg -Type Error -MSG $Error[0]
	}
...
Function Get-LogonHeader
{
	...
	catch
	{
		...
		#Write-Verbose $_.Exception.Response.StatusDescription
		$logonToken = ""
	}
    If ([string]::IsNullOrEmpty($logonToken))
    {
        Write-Verbose "Logon Token is Empty - Cannot login"
        exit
    }
...
if(-not(Test-Path $CsvPath)){
	Log-MSG "Cannot onboard accounts. File '$CsvPath' does not exist"
	return
}

if($null -ne $ConfigFile){
	Log-MSG "Loading config file '$ConfigFile'"
	#$config = LoadAccountsFeedConfig -configpath $ConfigFile
	$VaultConfig = (LoadJSONConfig -configpath $ConfigFile).VaultAuthorization
	$config = (LoadJSONConfig -configpath $ConfigFile).AccountOnboardingUtility
	$UseConfig = $null -ne $config
	...
	$TemplateSafe = $config.TemplateSafe
	#$CsvPath = $config.CsvPath
	# if($null -eq $Filter){
	# 	Log-MSG "Must define a filter for file selection"
	# 	return
	# }
	
	#$OnboardFile = ($config.Files | Where-Object -FilterScript $Filter)

	#$CsvPath = $OnboardFile.CsvPath
	Log-MSG "Onboarding file '$CsvPath'"
	$Username = $VaultConfig.user
	$DisableSSLVerify = $config.DisableSSLVerify -eq "true"
	$Create = $config.Create -eq "true"
	$Update = $config.Update -eq "true"
	$Delete = $config.Delete -eq "true"
	$NoSafeCreation = $config.NoSafeCreation -eq "true"
	$Force = $config.Force -eq "true"
	...
	if($null -eq $Password){
		Log-MSG "Getting vault logon credentials for $Username"
		...
		if($null -eq $appUserCred) {
			Log-MSG "Could not get report app user credential!" 
	...
	if($null -eq $Password){
		Log-MSG "Password is null. Passing a configuration file requires a password for the referenced username attribute."
		return
	}

}else{
	Log-MSG "No config file found."
}
# Check if to disable SSL verification
If($DisableSSLVerify)
{
	try{
		if($UseConfig -eq $false){
...
#region [Logoff]
	# Logoff the session
    # ------------------
    Write-Verbose "Logoff Session..."
    Invoke-Rest -Uri $URL_Logoff -Header $g_LogonHeader -Command "Post"
	# Footer
	
	$ArchiveFile = $false
	$ArchiveFolderExists = $false
	Log-MSG "Preparing to archive if applicable..."
	if($null -ne $ArchiveFolder){
		$ArchiveFolderExists = Test-Path $ArchiveFolder
	}
	if($ArchiveFolderExists){
		if($counter -eq $rowCount){
			$ArchiveFile = $true
		}else{
			if($SkippedDueToError){
				Log-MSG "Will NOT archive '$CsvPath'. Accounts were skpped due to errors."
			}else{
				$ArchiveFile = $true
			}
		}
		...
		if($null -ne $ArchiveFolder){
			Log-MSG "Will NOT archive '$CsvPath'. Archive folder does not exist."
		}
		
	}
	if($ArchiveFile){
		$ArchiveFileName = Split-Path $CsvPath -leaf
		try {
			if((Test-Path $CsvPath)){
				$ArchiveFilePath = Join-Path -Path $ArchiveFolder -ChildPath $ArchiveFileName
				Log-MSG "Archiving file '$ArchiveFileName' to '$ArchiveFolder'"
				Copy-Item $CsvPath $ArchiveFilePath
				Remove-Item -Path $CsvPath
				Log-MSG "File '$CsvPath' archived to '$ArchiveFolder'."
			}else{
				Log-MSG "Archive is bypassed. File '$CsvPath' does not exist."
			}
			
		}catch{
			Log-Msg -Type Info -MSG "Could not archive file '$CsvPath'. Error: $(Collect-ExceptionMessage $_.Exception)"
		}
		

	}else{
		Log-MSG "Will not archive due to errors, no archive folder specified or archive folder not found."
	}
