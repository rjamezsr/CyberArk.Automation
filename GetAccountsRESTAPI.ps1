#region About GetAccountsRESTAPI.ps1
###########################################################################
...
function EnumerateResults {
    param($AuthZHeader,$TargetSafe,$offset=1,$limit=1000,$urlSuffix="")
    $results = $null
    $nextLink = $null
    $rows = 0
    if($urlSuffix.Length -gt 0){
        $GetAccountsURI = $PVWAURL+"/"+$urlSuffix
    }else{
        $GetAccountsURI = $PVWAURL+'/api/Accounts?filter=safeName eq {0}&offset={1}&limit={2}'
        $GetAccountsURI = $GetAccountsURI -f $TargetSafe, $offset, $limit
    }
    
    $results = GetAccounts -authZtoken $AuthZHeader -TargetSafe $TargetSafe -cyberarkGetAccountsURL $GetAccountsURI
    
    if($null -ne $results){
        $rows = $results.value.Count
        LogMessage -message "**********************************"
        LogMessage -message "**********************************"
    ...
    <#
        ...
        if($rows -eq $limit){
            $newOffset = $rows + $limit
            EnumerateResults -AuthZHeader $AuthZHeader -TargetSafe $TargetSafe -offset $newOffset -limit $limit
        }
    }
    #>
}
...
if($null -eq $TargetSafe -or $TargetSafe.Length -eq 0){
    ...
    if($null -eq $config.GetAccountsRESTAPI.Safes){
        ...
        $TargetSafes = $config.GetAccountsRESTAPI.Safes
    }
...
$SafeListReference = $config.GetAccountsRESTAPI.SafeListReference
...
$TargetSafes | Foreach-Object {
    ...
    $GetSafesListURI = $PVWAURL+'/api/safes?search={0}&limit=100000'
    ...
    if($null -ne $IgnoreSafes -and $IgnoreSafes -ne ""){
        $SafesList       = $SafesListFull.$SafeListReference | Where safeName -notlike "*$IgnoreSafes*"
    }else{
        $SafesList       = $SafesListFull.$SafeListReference 
    }
    ...
    if($NumberOfSafes -gt 0){
        ...
        $SafesList | Select safeName | ForEach-Object {
            ...
            $progress = Show_Progress -progressObject $progress -list $SafesList -message "Query safe $TargetSafeName"
            ...
            $ResultsCollection | ForEach-Object {
                ...
                $FileExists = Test-Path $sourcepath
                if($FileExists){
                    $Accounts.value | Select -Property $ReportColumns | Export-CSV $sourcepath -Append -NoTypeInformation
                }else{
                    $Accounts.value | Select -Property $ReportColumns | Export-CSV $sourcepath -NoTypeInformation
                }
            }
    ...
    if($filetoUploadExists){
        if($null -ne $ReportSafe -and $ReportSafe -ne ""){
            $ReportFiles.Add([pscustomobject]@{
                File=$filetoUpload;
                Safe=$ReportSafe;
                Folder=$ReportFolder;
            })
...
if($null -ne $AuthZHeader -and $AuthZHeader -ne ""){
    Logoff -authZtoken $AuthZHeader -BaseURL $config.RESTAPI.BaseURL
}
